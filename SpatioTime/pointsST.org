...
- the =sp= and =spacetime= packages (with the aid of =zoo= and =reshape2=),
...
#+CAPTION: Scatterplots of the $NO_2$ values (2011) with a panel for each day of the time series. Each circle represents a different station. label:fig:NO2STxy
...
space on the x-axis and time on the y-axis (Figure [[ref:fig:NO2hovmoller]]).
...
#+CAPTION: Space-time graphic of the NO_2 time series. Each column represents a different station (denoted with the last two digits of the code). label:fig:NO2hovmoller
...
#+CAPTION: Time graph of the $NO_2$ time series (2011). Each line represents a different station. label:fig:NO2zoo
...
This section details a method to create an animation with point data
built over the functionalities of the =gridSVG= package. We will work
with the =NO2st= object previously described in the section
ref:sec:intro_pointsST.
