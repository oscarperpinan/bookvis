...
The main packages in this section are: =raster= and =terra= are used for reading and manipulating the raster data, =sf= for reading spatial vector data, =rnaturalearth= and =geodata= for retrieving data from the NaturalEarth and DIVA-GIS services, =viridisLite= and =RColorBrewer= for palettes of colours, and =rasterVis= for producing the graphics.
...
context with a layer of administrative boundaries. These boundaries
can be retrieved from the Natural Earth service with the
...
longitude (=cut(x, 5)=), using the =magma= palette contained in the
=viridisLite= package. It is evident that larger irradiation values
...
The main packages in this section are: =raster= and =terra= are used
for reading and manipulating the raster data, and =rasterVis= for
