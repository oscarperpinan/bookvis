...
boundaries <- ne_countries(country = "spain", scale = 50)
...
- Download a Digital Elevation Model (DEM) from the DIVA-GIS service with the =geodata= package.
...
- Compute the hill shade raster with =terrain= and =shade= from
  =terra= with a certain angle and direction.
...
#+begin_src R :eval no-export
DEMr <- raster(DEM)
sloper <- terrain(DEM, "slope")
aspectr <- terrain(DEM, "aspect")
