...
GeoJSON and KML formats; 3D visualization with the =rgl= package; and
=SVG= graphics with the =gridSVG= package.
...
commonly displayed as a tooltip when the mouse hovers over the element
(line [[(titleTooltip)]]). The content of this attribute can be modified
thanks to the =grid.garnish= function. Moreover, the =grid.hyperlink=
function can add hyperlinks to the correspondent graphical element.

The tooltips will display the photography of the station (Section
ref:sec:photographs_stations, line [[(photo)]]), the name of the station,
and the statistics previously calculated with =aggregate= in the first
step of this chapter (line [[(stats)]]).
...
tooltips <- sapply(seq_len(nrow(NO2df)), function(i)
{
    ...
    stats <- paste(c('Mean', 'Median', 'SD'),                       (ref:stats)
                   signif(NO2df[i, c('mean', 'median', 'sd')], 4),
                   sep = ' = ', collapse = '<br />')
    ## Station photograph 
    imageURL <- paste('images/', codEst, '.jpg', sep = '')          (ref:photo)
...
grid.garnish('points.panel',
             title = tooltips,                              (ref:titleTooltip)
             grep = TRUE,
             group = FALSE)
#+end_src
