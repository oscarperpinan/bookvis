...
label:sec:administrative_boundaries
#+begin_src R :exports none
##################################################################
## Administrative boundaries
##################################################################
...
the National Statistics Institute (INE) of Spain webpage[fn:7]. The
municipalities, =sfMun=, are read with =st_read= of the =sf= package.
...
The INE also publishes data on population[fn:8] and Gross Domestic
Product (GDP)[fn:12] for the provinces of Spain. The result after
cleaning the data of population is stored in the file =PopSpain.csv=
and after filtering the data of GDP for the year 2020 is stored in the
file =GDPSpain2020.csv=.

#+INDEX: Data!INE
#+INDEX: Data!Population
#+INDEX: Data!Gross Domestic Product
#+INDEX: Subjects!Data processing and cleaning
...
The province boundaries are stored in the =spain_provinces_2.shp=
file, retrieved from the webpage of the INE (Section
ref:sec:administrative_boundaries).
...
sfProv <- st_read("data/Spatial/spain_provinces_2.shp", crs = 25830,
                 stringsAsFactors = TRUE)
#+end_src
Finally, the data frame and the provinces boundaries are merged together:
#+begin_src R
...
* Footnotes
[fn:12] https://www.ine.es/jaxi/Tabla.htm?tpx=67284&L=0 

[fn:8] https://www.ine.es/jaxiT3/Tabla.htm?t=2852&L=0
...
[fn:1] https://www.eea.europa.eu/soer/2015/countries/spain
