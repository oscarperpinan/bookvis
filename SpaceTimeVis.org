...
highlighted here:
 ...
 - =raster= cite:Hijmans2024 is a major extension of gridded spatial data classes. It provides a unified access method to different raster formats, permitting large objects to be analyzed with the definition of basic and high-level processing functions (Sections ref:sec:raster and ref:sec:rasterST). =rasterVis= cite:Perpinan.Hijmans2023 provides enhanced visualization of raster data with methods for spatiotemporal rasters (Sections ref:sec:rasterVis and ref:sec:rastervisST).
...
\nomenclature{GIS}{Geographic Information Systems.}
 ...
 ref:cha:cartogram, ref:cha:vector, and ref:cha:raster focus on
 thematic maps, that display a specific variable commonly using
 ...
 specific information about particular locations or areas
 (proportional symbol mapping, choropleth and cartogram maps) and
 information about spatial patterns (isarithmic and raster maps).
...
#+INDEX: Packages!rasterVis@\texttt{rasterVis}
 ...
 The =rasterVis= package cite:Perpinan.Hijmans2023 complements the =raster= package, providing a set of methods for enhanced visualization and interaction. This package defines visualization methods (=levelplot=) for quantitative data and categorical data, both for univariate and multivariate rasters.
...
\chapterprecishere{%
  ...
  The most relevant packages used in this chapter are: \texttt{raster} and \texttt{terra}
  for reading raster data; \texttt{rasterVis} and \texttt{ggplot2} for visualization of
...
\chapterprecishere{%
  ...
  data from different sources. Next subjects are covered in this chapter: label positioning, hill shading, and map overlaying.

  The most relevant packages used in this chapter are: \texttt{raster} and \texttt{terra}
  for reading raster data;\texttt{rasterVis} and \texttt{tidyterra} for visualization of
  raster data; \textttt{rnaturalearth} and \textttt{geodata} for retrieving spatial data.%
}
#+END_EXPORT
...
\chapterprecishere{%
  ...
  The most relevant packages used in this chapter are: \texttt{raster} and \texttt{terra}
  for reading raster data;\texttt{rasterVis} and \texttt{tidyterra} for visualization of
  raster data; and \texttt{osmdata} for retrieving data from OpenStreetMap.
  %
}
#+END_EXPORT
...
*** Packages
 ...
 The CRAN Tasks View "Handling and Analyzing Spatiotemporal Data" [fn:31] summarizes the packages for reading, vizualizing, and analyzing space-time data. This section provides a brief introduction to the =spacetime=, =sf= and =sftime= , =raster=, =terra=, =rasterVis=, =rgl=, and =gganimate= packages. Most of the information has been extracted from their vignettes, webpages, and help pages. You should read them for detailed information.
...
#+INDEX: Packages!raster@\texttt{raster}
 ...
 The =raster= package cite:Hijmans2024 is able to add time information associated with layers of a =RasterStack= or =RasterBrick= object with the =setZ= function. This information can be extracted with =getZ=.
...
#+INDEX: Packages!rasterVis@\texttt{rasterVis}
 ...
 =rasterVis= cite:Perpinan.Hijmans2023 provides three methods to display spatiotemporal rasters:
...
*** Further Reading
 ...
 -  The CRAN Tasks View "Handling and Analyzing Spatiotemporal Data" summarizes the packages for reading, vizualizing, and analyzing space-time data. The R-SIG-Geo mailing list [fn:33] is a powerful resource for obtaining help.
