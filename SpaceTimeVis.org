...
*** What You Will /Not/ Find in This Book
 ...
 - *Finally, this book is not a handbook of data analysis, geostatistics, point pattern analysis, or time series theory*.
   ...
   Instead, this book is focused on the exploration of data with visual methods, so it may be framed in the Exploratory Data Analysis approach. Therefore, this book may be a useful complement for superb bibliographic references where you will find plenty of information about those subjects. For example, cite:Chatfield2003, cite:Cressie.Wikle2011, cite:Slocum.McMaster.ea2005 and cite:Bivand.Pebesma.ea2013.
...
**** Website and Code Repository
 ...
 The repository and the website can be downloaded as compressed files[fn:13], and if you use =git=, you can clone the repository with
...
highlighted here:
 ...
 - =raster= cite:Hijmans2013 is a major extension of gridded spatial data classes. It provides a unified access method to different raster formats, permitting large objects to be analyzed with the definition of basic and high-level processing functions (Sections ref:sec:raster and ref:sec:rasterST). =rasterVis= cite:Perpinan.Hijmans2017 provides enhanced visualization of raster data with methods for spatiotemporal rasters (Sections ref:sec:rasterVis and ref:sec:rastervisST).
...
In Section ref:sec:differentVariables we learned to display the time evolution of multiple time series with different scales. But, what if instead of displaying the time evolution we want to represent the relation between the variables? Section ref:SEC:groupVariable proposes the scatterplot matrix solution using groups defined according to the time as a grouping variable. Section ref:SEC:conditionVariable uses an enhanced scatterplot with time as a conditioning variable. Section ref:SEC:hexbin includes a discussion about the hexagonal binning for large datasets.
...
Gapminder [fn:17] is an independent foundation based in Stockholm, Sweden.  Its mission is "to debunk devastating myths about the world by offering free access to a fact-based world view." They provide free online tools, data, and videos "to better understand the changing world." The initial development of Gapminder was the Trendalyzer software, used by Hans Rosling in several sequences of his documentary "The Joy of Stats."
...
\nomenclature{GIS}{Geographic Information Systems}
 ...
 This part exposes several examples where vector and raster data are displayed. These examples show geographic location of features (reference maps, Chapter ref:cha:refer-phys-maps), physical landscape features of a place (physical maps, Chapter ref:cha:refer-phys-maps), or a specific variable in the context of a geographic reference (thematic maps, Chapters ref:cha:bubble, ref:cha:choropleth, and ref:cha:raster). These examples make use of several datasets (available at the book website) described in Chapter ref:cha:dataSpatial.
...
#+INDEX: Packages!rgdal@\texttt{rgdal}
 ...
 The =rgdal= package cite:Bivand.Keitt.ea2013 provides bindings to the Geospatial Data Abstraction Library (GDAL) [fn:21]. With =readOGR= and =readGDAL=, both GDAL raster and OGR vector map data can be imported into =R=, and GDAL raster data and OGR vector data can be exported with =writeGDAL= and =writeOGR=.
...
** Physical and Reference Maps
 ...
 This chapter details how to create a physical map of Brazil with data from different sources, and a reference map of a northern region of Spain using data from OpenStreetMap.
...
[fn:17] [[http://www.gapminder.org/]]

[fn:13] Repository: https://github.com/oscarperpinan/spacetime-vis/archive/master.zip, Website:  https://github.com/oscarperpinan/spacetime-vis/archive/gh-pages.zip
...
[fn:1] http://learnr.wordpress.com/2009/06/28/ggplot2-version-of-figures-in-lattice-multivariate-data-visualization-with-r-part-1/
