...
** Displaying Spatiotemporal Data: Introduction
 ...
 The next chapters, focused on areal space-time data (Chapter ref:cha:arealST), raster space-time data (Chapters ref:cha:rasterST and ref:cha:animationST) and point space-time data (Chapter ref:cha:pointsST), illustrate with examples how to produce animations, multipanel graphics, hovmöller diagrams, and time-series with =R=.
...
\chapterprecishere{%
  ...
  \texttt{zoo}, \texttt{spacetime}, and \texttt{sftime} for reading and defining point
...
\chapterprecishere{%
  ...
  The most relevant packages in this chapter are: \texttt{raster} and \texttt{terra} for
...
\chapterprecishere{%
  ...
  also be constructed from the evolution of the position of a moving
  object (Section \ref{sec:trajectory}), from the sequence of changes
  in an attribute of the variable, for example, the user's view of an
  static variable can be gradually moved to create a fly-by animation
  (Section \ref{sec:animationST_3d}), and with the hill shading
  technique changing the sun position over time (Section
  \ref{sec:sun3d}).

  The most relevant packages in this chapter are: \texttt{raster} for
  reading raster data; \texttt{sf} and \texttt{sp} for reading spatial
  vector data; \texttt{zoo} for time series data; \texttt{rasterVis}
  for visualization of raster data; \texttt{rnaturalearth},
  \texttt{osmdata} and \texttt{move2} for retrieving data;
  \texttt{rgl} for 3D visualization; \textttt{gganimate} for animation
  with \texttt{ggplot2}; and \texttt{gridSVG} for interactive
  graphics.
  %
}
#+END_EXPORT
